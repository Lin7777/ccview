...
def preprocess_data(data):
    ...
    features = data.drop(columns=['Risky_Crew', '总排名', '总排名百分比'])
...
def analyze_feature_importance(results, features_list):
    feature_importances = {}
    for model_name, result in results.items():
        model = result['model']
...
def tune_decision_tree(X_train, y_train, X_test, y_test, features_list):
...
def main():
    # Load data
    file_path = '/Users/jl/PycharmProjects/ccviewandwarning/test_code/data/cc_data.xlsx'
    data = pd.read_excel(file_path)

    # Preprocess data
    features_scaled, target, features_list = preprocess_data(data)
    ...
    analyze_feature_importance(results, features_list)

    # Tune Decision Tree model
    tune_decision_tree(X_train, y_train, X_test, y_test, features_list)
